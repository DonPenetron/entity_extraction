...
ENTITY_TYPE = "strategic_aim"


# class StrategicAimParser:
#     def __init__(self, syntax, verbose: bool = False):
#         self.free_entity_parser = FreeEntityParser(syntax, verbose)

#     def __call__(self, df: pd.DataFrame):
#         return {
#             "free_entity": self.free_entity_parser(df),
#             "anchor_entity": []
#         }
...
#                 if rparser(row["text"]) is not None and (is_bold(row.to_dict()) or is_italic(row.to_dict())):
#                     is_found = True
#                     break
#             if is_found:
#                 indices.append(i)
...
re_rules_free = [
    {
        "pattern": r"Стратегическая[ ]{1,5}цель[ ]{1,5}\d[ ]{,5}[.]",
        "action": "match",
